...
logger = logging.getLogger('fms.engines.asynchronousrandwreplaceexpirablemultiple')

class AsynchronousRandWReplaceExpirableMultiple(Engine):
    """
    ...
    def run(self, world, agents, market):
        ...
        for day in range(self.days): 
            for time in range(self.daylength):
                ...
                order = market.sanitize_order(agents[agt].speak(market)) # TODO: create a new engine/market that handles agents sending multiple orders. Need to allow `speak` to return a list of orders and need to change `sanitize_order` to accept a list of orders. And also to sanitize against wash trades. 
