...
class ChiarellaIori(agents.Agent):
    ...
    def __init__(self, params, offset=0):
        ...
        try:
            self.n = self.args[2] # noise weight
        except IndexError:
            raise MissingParameter, 'n'
        try:
            self.k_max = self.args[3] # order tolerance 
        except IndexError:
    ...
    def act(self, world=None, market=None):
        ...
        if self.price_forecast > market.lastprice:
            ...
            return {'direction':BUY, 'price':bid, 'quantity':1}
        else:
            ask = (1. + self.k) * self.price_forecast 
            return {'direction':SELL, 'price':ask, 'quantity':1}
